...
1) 1行目が命：短く・強く・言い切り（12〜25文字目安）。スクロールを止める10の型から選ぶ：
   ...
   ③ 悩みを地の文で言い当てる（「朝起きた瞬間、腰が固まってる」）— 同意を求める確認疑問（「〜していませんか？」）にはしない
   ④ カッコいいセリフ（印象的なフレーズ）
...
- **1行目は"お客さんが心の中で呟いている言葉"をそのまま置く**：「腰痛に悩む方へ」より「靴下を履くとき、片足で立つのがこわい」。感情が動いた具体的な瞬間（いつ・どこで）まで掘り、**地の文の言い切り**で書く（同意を求める確認疑問「〜してませんか」の形にはしない・禁止）。
...
- 投稿の締めは、次のいずれかの「開かれた終わり方」を基本にする：
  ・軽い問いかけ（例の方向性：「みなさんはどっち派ですか？」「朝と夜、どちらが痛みますか？」）— 読者が一言で答えられる、本当に答えを知りたい質問だけ
  ・共感の相づちを打ちたくなる一言（例の方向性：「これ、うちだけじゃないはず」）
  ・読者の状況を気づかう一言（例の方向性：「今日はゆっくり休んでください」）
  ※「〜いませんか？」「〜と思いませんか？」「気になりませんか？」のような**同意を求める確認疑問は禁止**（人間の投稿には出ない機械だけの形）。上の例の言い回しをそのまま投稿に写さない。
...
- 1行目は悩みを**地の文で言い当てる**（「夕方になると足がパンパンになる」）。「〜で悩んでいませんか？」のような同意を求める確認疑問では始めない（禁止）
- お客さんの言葉をそのまま使う（N1分析）
- 「わかります」「実は私も〜」で共感
- 解決策は最後に軽く触れる程度
`,
...
- 数字は**入力情報に明記されているものだけ**使う（初回価格・所要時間など）。入力に無い「初回3,980円」のような数字は捏造しない
- **「先着◯名」「残り◯枠」「お早めに」のような、確かめようのない希少性・緊急性は書かない**（景表法リスク・2026-08-28の採否判断で不採用）
...
- 1行目はお客様の不安を**地の文で**そのまま言葉にする（「体が硬いから自分には無理、とよく言われます」）。「〜と思っていませんか？」のような同意を求める確認疑問にはしない（禁止）
- 「実は逆」「むしろ〜」で安心を渡す
