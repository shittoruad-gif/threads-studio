/**
 * Threads Studio 営業/紹介用スライド生成（整骨院・整体院向け）
 *   node scripts/make-pitch-slides.cjs
 * 出力: threads-studio-pitch-seikotsu.pptx
 */
const pptxgen = require('pptxgenjs');

const pres = new pptxgen();
pres.layout = 'LAYOUT_16x9'; // 10" x 5.625"
pres.title = 'Threads Studio — 整骨院・整体院向け';
pres.author = '株式会社しっとる';

// ─── パレット（医療系・信頼感のある紺＋白＋アクセント緑）─────────
const C = {
  navy: '1E3A5F',
  navyDark: '152B47',
  white: 'FFFFFF',
...
// ════════════════════════════════════════════════════════════════
{
  ...
  // 3つのスタットチップ
  const stats = [
    { big: '月¥4,980〜', small: '個人院でも始められる価格' },
...
// ════════════════════════════════════════════════════════════════
{
  ...
  pageHeader(s, '整骨院オーナーが抱える集客の悩み', 'こんな状況、思い当たりませんか？');

  const items = [
    { t: '既存顧客頼りで頭打ち', d: '紹介とリピートだけでは限界が見えてきた' },
    { t: '新規が安定して来ない', d: '広告は費用が見合わない／SNSは何を書けばいいかわからない' },
    { t: '法令で書きたいことが書けない', d: '「治る」「効く」が使えず、投稿の手が止まる' },
    { t: '投稿する時間がない', d: '施術と経営で手一杯、毎日の文章作成は現実的でない' },
    { t: '競合の中で埋もれる', d: '近所に整骨院が増え、選ばれる理由を発信できていない' },
  ];
  const baseY = 1.65;
  const rowH = 0.65;
  items.forEach((it, i) => {
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, 'なぜ整骨院のSNS運用は続かないのか', '止まる原因は、ほぼ次の3つです');
  ...
  const walls = [
    {
      ...
      d: '何を書くか考える時間、規制に違反していないか確認する時間、誤字脱字チェック ── 1本作るのに30分。週5本で月10時間。施術の合間にはまず確保できない。',
    },
    {
      label: '規制の壁',
      icon: '法',
      title: '「治る」「効く」が書けない',
      d: 'あはき法・柔整法・医師法・薬機法。どこまでが安全な表現か、明確な線引きを毎回判断するのは現実的ではない。結果、当たり障りのない投稿になり、誰にも刺さらない。',
    },
    {
      label: 'センスの壁',
      icon: 'セ',
      title: '「何が伸びるか」がわからない',
      d: '1行目に何を書くか、地域名はどこに入れるか、ハッシュタグは何個か。アルゴリズムは変わり続け、感覚で書いても伸びない。誰かに聞こうにも、業界に詳しい人がいない。',
    },
  ];
...
// ════════════════════════════════════════════════════════════════
// 5. なぜThreadsが整骨院に向くのか
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, 'なぜ Threads が整骨院に向くのか', '2026年時点の集客特性 ── 他SNSと違うポイント');

  const reasons = [
    { t: 'テキスト主体で十分に伸びる', d: '画像/動画を作り込まなくても、文章だけで届く。\n撮影機材も加工技術も不要。' },
    { t: '地域おすすめが強い', d: '1行目に地域名を入れると、近くに住む人の\nタイムラインに優先表示される仕様。' },
    { t: '近隣到達でCVに直結', d: '万バズは不要。近所の数百〜数千人に\n確実に届けば、来院に繋がる。' },
    { t: '広告費ゼロでの新規獲得', d: '投稿だけで集客できる。広告予算が\n限られる小規模院に最適。' },
  ];
  ...
  // 補足
  s.addText('※ Instagramは写真前提・Xは速報前提。整骨院の「地域×ストーリー」と最も相性が良いのがThreadsです。', {
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const pillars = [
    { n: 'ネタ作り', sub: 'AIが文章を生成', d: '院の業種・地域・強みからAIが投稿文を生成。13種類の型から選べる。共感／実績／地元ネタ／Q&A／ストーリー…' },
    { n: '運用代行', sub: '自動で毎日投稿', d: '毎日決まった時間に自動投稿。手を動かさなくても発信が続く。スマホを開かない日があっても勝手に投稿される。' },
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '院オーナーが得られる 5つの具体的メリット', '感覚ではなく、数字で変わるもの');

  const benefits = [
    { num: '1', big: '1投稿 20分 → 2分', label: '時間削減', d: '考える・書く・規制チェックの工程をAIが代行。月20本投稿なら 6時間 → 40分。' },
    { num: '2', big: '代行費 月5〜30万 → ¥4,980〜', label: 'コスト削減', d: 'SNS代行は月5〜15万、Threads特化なら月15万＋初期費10万が相場。1/10以下で、しかも院の言葉で書ける。' },
    { num: '3', big: '15業界の法令を内蔵', label: 'リスク回避', d: '広告規制違反のリスクをAI側で自動回避。「うっかり違反」を構造的に防ぐ。' },
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '導入前と導入後 ── 1日の流れはこう変わる', '個人経営の整骨院（夜診あり・1人運営）を想定');
  ...
  const beforeRows = [
    { t: '朝',   d: '「今日も投稿しなきゃ」と思いつつ、何を書くか思いつかない' },
    { t: '昼',   d: '昼休みの15分でネタを探し、3行書いて止まる' },
    { t: '夕方', d: '「治る」と書きそうになり、消す。当たり障りない文に' },
    { t: '夜',   d: '結局投稿せず、SNSは月に2〜3回しか動かない' },
    { t: '結果', d: '新規来院は紹介頼みのまま。SNSからの来院ゼロ' },
  ];
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '差別化①  業界規制を内蔵チェック', 'あはき法・柔整法・医師法・薬機法に対応した自動回避');
  ...
  const ng = ['治る', '治す', '治療', '効く', '改善', '完治', '〇〇病が治る', '日本一', '絶対'];
  ng.forEach((w, i) => {
  ...
  s.addText('整骨院／整体／鍼灸院は法令上、「治療」「治る」「効く」が使えません。', {
  ...
  const ok = ['ケア', '施術', 'サポート', 'アプローチする', '整える', '楽になる方が多い', '丁寧に見る', 'こだわっています', '可能な範囲で'];
  ok.forEach((w, i) => {
    const col = i % 3, row = Math.floor(i / 3);
    s.addText(`○ ${w}`, {
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '差別化②  事実ベース生成（捏造ゼロ）', '架空の患者・盛った実績を、AIに作らせない');
  ...
  s.addText('「実績は無い」と答えた院では、AIは数字を出しません。「患者エピソードは無い」と答えれば、架空の物語を作りません。', {
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const right = [
    { t: 'バズより近隣到達', d: '万単位の拡散は不要。近所の数百人に届けば来院に繋がる' },
    { t: '返信を生む投稿設計', d: 'いいねよりコメントが評価される時代。問いかけで終わる構成' },
    { t: 'リアルタイム実況型を搭載', d: '「今日はこんな症状の方が」など現場感のある投稿パターン' },
  ];
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '実際にAIが生成する投稿サンプル', '「横浜市港北区／整体院／在宅ワークの30代女性向け」と入力した場合');

  // 入力サマリ
  card(s, 0.7, 1.6, 8.6, 0.7, { fill: 'EEF2FF' });
  s.addText('業種: 整体院  /  地域: 横浜市港北区  /  ターゲット: 在宅ワークの30代女性  /  型: 共感型', {
  ...
  s.addText(
    '横浜市港北区で在宅ワーク中の方へ。\n\n夕方になると肩と首がガチガチで、頭痛まで出てくる。\nそんな日が続いていませんか？\n\n当院は完全予約制の個室で、女性スタッフが対応。\n着替え不要、デスクワーク姿勢のクセを丁寧に見ます。\n\n「ちょっと様子を見てもらいたい」段階で大丈夫です。',
    {
...
// ════════════════════════════════════════════════════════════════
{
  const s = pres.addSlide();
  s.background = { color: C.bgSoft };
  pageHeader(s, '他の選択肢との比較', '院オーナーが実際に選びそうな4つの方法');
  ...
  s.addText('※ 代行業者の月額相場は投稿のみで5万円〜、Threads特化型・Instagram治療院向け代理店で15〜30万円が一般的（2026年5月現在の公開価格を参照）。', {
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const options = [
    {
      ...
      cost: '月 ¥60,000〜100,000相当',
      detail: '院長の時間単価は施術単価で換算。\n¥6,000〜10,000/h × 月10時間。\nその時間を施術にあてれば\n10人分の売上機会。',
      verdict: '機会損失が大きい',
      verdictColor: 'B91C1C',
    },
    {
  ...
  s.addText('院長の1時間は施術1人分の売上。スタッフの1時間は給与。── 月¥4,980 はどちらより圧倒的に安い。', {
    x: 0.7, y: 4.9, w: 8.6, h: 0.32,
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const steps = [
    ...
    { n: 'STEP 3', t: 'プロジェクト作成', d: '院の業種・地域・ターゲット・主な悩み・強みを入力' },
    { n: 'STEP 4', t: 'AIカウンセリング（8問）→ 生成開始', d: '質問に答えるとAIがその院の言葉で投稿を作り始めます' },
  ];
...
// ════════════════════════════════════════════════════════════════
{
  ...
  const safety = [
    ...
    },
    {
      ...
      d: '院ごとにプロジェクトが分離。投稿内容・カウンセリング回答は他社・他院に共有されません。AIの学習には使用されません（外部AI API利用時の設定を含む）。',
    },
    {
      label: 'コスト保護',
  ...
  const qas = [
    {
      ...
      a: '8問のカウンセリングで院の実情を聞いてから作るので、答えていない数字や架空エピソードはAIが作りません。「事実だけ」で書く設計です。',
    },
    {
      q: '広告規制に違反しないか心配です',
      a: '業種を「整骨院／整体／鍼灸」と入れた時点で、対応法令（あはき法・柔整法・医師法・薬機法）の禁止表現がAI側で自動回避されます。「治る」「効く」が出ない仕組みです。',
    },
    {
      q: '忙しくて毎日投稿する余裕がありません',
      a: '自動投稿スケジューラを使うと、毎日決まった時間にAIが投稿を作って自動送信します。週1回ログインして内容を確認するだけでも運用が回ります。',
    },
    {
      q: '効果は本当にありますか？',
      a: '個別の効果保証はしていません（医療系の集客に「必ず」は禁物）。ただし、地域名×具体性の投稿が近隣のおすすめに乗りやすい仕組みは2026年のThreadsで一貫しています。',
    },
  ];
...
pres.writeFile({ fileName: 'threads-studio-pitch-seikotsu.pptx' })
