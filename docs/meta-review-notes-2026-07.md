...
## 2026-08-20: 6回目の提出（submission_id 1404421808262189）

ステータス「審査中」（20日以内）。3権限＋更新2権限。

### 提出物
- 動画: `~/Documents/meta_review_2026-08/seg_final_small.mp4`
  （1920x920 / 13分02秒 / 6.8MB / 英語キャプション焼き込み）
  素材: seg1_connect_publish_raw.mp4（15分）+ seg2_replies_insights_raw.mp4（7分）
  → 上部160pxクロップ（タブバー・デバッグバナー除去）→ 結合 → PILで生成した
     字幕PNGをoverlayで合成 → fps15/crf30で10MB以下に圧縮
- 3権限すべてに同一動画を添付。申請ノートの先頭に「前回却下への回答＋秒数」を追記。
- 審査担当者の指示も全面的に書き直し（変更点・秒数・自分でテストする手順）。

### 動画の中身（実測・フレーム画像で確認済み）
```
00:32  Threadsのログイン画面（ID/パスワード入力）
01:02  権限同意画面（5権限を一覧表示）
01:22  「shittoru_officialとして続行」で権限付与
01:32  アプリに戻り接続完了（3/3）
04:00  AI投稿を生成・公開（content_publish）
07:50  Threads上で公開を確認
09:20  コメントへのAI返信・公開（manage_replies）
11:10  インサイト取得・表示（manage_insights）
```

### 今回できたこと・つまずいたこと
1. **同意画面を出すには認可の取り消しが必要**。アプリ内Disconnectでは不十分で、
   Threads設定 →「ウェブサイトのアクセス許可」→ 削除、が必要。
   認可済みだと `auth_type=reauthenticate` でもショートカット表示になる。
2. **ログイン画面も出すにはThreadsからログアウトが必要**。認可取消だけでは
   同意画面しか出ない（セッションが生きているため）。
3. **投稿にはThreadsテスター登録が必須**。content_publish未承認のため、
   テスター未登録のアカウントは `Failed to create media container` で弾かれる。
   アプリの役割 → Threadsテスター追加 → アカウント側で招待を承諾。
4. **screencapture の停止は SIGINT**。`pkill`（SIGTERM）だとファイルが
   書き出されず録画が消える。2回失っている。自然終了を待つのが最も安全。
5. **Chromeが別ディスプレイにあることがある**。`screencapture -D 3` のように
   ディスプレイ指定が必要。事前に各ディスプレイをキャプチャして確認する。
6. **字幕の焼き込み後は必ずフレーム画像で照合する**。合成スクリプトが古い
   時刻設定のままで、ログイン画面に別の説明文が乗っていた（＝前回の却下と
   同じ状態）。提出前に発見して修正した。
