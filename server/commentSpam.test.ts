...
describe("勧誘コメントのカード", () => {
  ...
  it("文案も、返信のボタンも出さない", async () => {
    const msgs: any[] = await buildCommentReplyCards([item]) as any[];
  ...
  it("ふつうのコメントには、文案と一緒に「返信しない」も出す", async () => {
  ...
  it("勧誘だけのときは、説明文に「この文で送る」の案内を出さない", async () => {
    const msgs: any[] = await buildCommentReplyCards([item]) as any[];
