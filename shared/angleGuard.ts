...
export const ANGLE_REQUIREMENTS: Record<string, AngleRequirement> = {
  ...
  future_worry: {
    ban: /(逆効果|意味(が)?ない|無駄です|繰り返します|繰り返してしまい|体(は|が)[^。\n]{0,8}変わ|必ず|治(る|り|ります|せ)|根本から|完治|改善します|ごまかし|ごまかす|その場しのぎ)/,
  ...
  misconception: {
    mark: /(誤解|勘違い|思われがち|思っている人|実は|じつは|本当は|もったいない|正しくは|違います|逆効果|意味がない|無駄|ではありません|じゃない|だけでは)/,
...
export function checkAngle(angleId: string | null | undefined, text: string): AngleCheck {
  ...
  if (req.ban && req.ban.test(t)) {
    ...
    const kind = /逆効果|意味|無駄|繰り返|変わ|必ず|治|根本|完治|改善|ごまか|その場しのぎ/.test(m) ? "効果・結果の言い切り" : "売り込み";
