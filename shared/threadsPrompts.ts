...
export interface PostScore {
  hookScore: number;      // 1行目の強さ（0〜25）
  valueScore: number;     // 価値提供の質（0〜25）
  ctaScore: number;       // CTAの明確さ（0〜25）
  targetScore: number;    // ターゲット適合度（0〜25）
 ...
 */
function buildSystemPrompt(treeCount: number, postType?: PostType, usp?: string, n1Customer?: string, purpose?: PostPurpose): string {
...
${purposeSection}${uspSection}${n1Section}
...
export function generateThreadsPrompt(input: ThreadsPromptInput): string {
  ...
  const systemPrompt = buildSystemPrompt(treeCount, input.postType, input.usp, input.n1Customer, input.purpose);
...
export function scorePost(mainPost: string, treePosts: string[], cta: string): PostScore {
  ...
  const total = Math.min(hookScore + valueScore + ctaScore + targetScore, 100);

  return {
    hookScore: Math.min(hookScore, 25),
    valueScore: Math.min(valueScore, 25),
    ctaScore: Math.min(ctaScore, 25),
    targetScore: Math.min(targetScore, 25),
    total,
