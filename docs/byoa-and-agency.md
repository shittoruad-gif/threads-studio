...
## 6. 代理店がクライアントに送る資料（2026-08-10）

- URL: `https://threads-studio.com/welcome` （**ログイン不要**）
- 実体: `client/src/pages/ClientWelcome.tsx`

### 構成（店舗オーナー向け・不安の先回り）
1. このサービスがすること … 投稿を書く / コメントに返す / 反応を数字で見せる
2. 最初にやる3つ … ログイン(1分) / Threads連携(30〜40分・手順書へリンク) / お店の情報(10〜15分)
3. 設定後は … 毎日の作業なし。週1回5分（コメント管理・投稿分析）
4. よくある質問 … **「AIが変なことを書かないか」に3つの実装で回答**
   （事実のみ使用・NGワード登録・公開前の承認モード）、毎日の作業要否、途中解約

### 代理店の運用
クライアント発行時の <ruby>案内文<rt>あんないぶん</rt></ruby>コピーに、この welcome / ログイン情報 / 設定手順URL を
1つのテキストとしてまとめてある。代理店は「コピー」を押してメールやLINEに貼るだけでよい。

3ページの関係:
- `/welcome` … 契約直後に読む「はじめに」（何ができる・何をする・不安の解消）
- `/threads-setup-guide` … 実作業の手順書（Facebook作成〜連携の全9ステップ）
- `/threads-setup-guide?mode=direct` … 上記＋登録とお支払い（直接契約のお客様向け）
