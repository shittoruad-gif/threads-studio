...
// 自動投稿の読みやすさ上限（スマホ前提）。
// スマホ画面で「スクロールせず全体が見える」のは合計200字程度まで。
// プロンプトでも指示するが、AIが超過した場合は段落単位で機械的に削る。
const AUTO_POST_CHAR_BUDGET = 220;
...
★スマホの画面で読まれる前提。「短く・行を割って・余白だらけ」が正解。
- mainPost は合計 **100〜160文字**。これを超えたら情報を削る。長い説明は書かない。
- **1行は全角15文字以内**。文の途中でも意味の切れ目で改行する（スマホの自動折り返しに任せない）。
- **1〜2行ごとに必ず空行**を入れる。画面が文字で埋まったら失敗。
- 1文は20文字以内を目安に短く割る。
- 1投稿1メッセージ。伝えることを1つに絞る。
- 1行目は15文字以内。「悩みの言語化」「共感の問いかけ」「意外な事実」のどれかで始める。
- 締めは読者が答えたくなる短い問いかけ1行。やわらかい敬語で（「同じ方いませんか？」）。タメ口（「どうしてる？」）は使わない。
...
【最優先：スマホでの見た目】
- 合計 **100〜160文字** に収める。長い場合は情報を削る（一番大事な1メッセージだけ残す）。
- **1行は全角15文字以内**。意味の切れ目で自分で改行する（自動折り返しに任せない）。
- **1〜2行ごとに必ず空行**。行間の余白で読ませる。画面が文字で埋まったら失敗。
- 1文は20文字以内を目安に短く割る。
...
async function generateAutoPost(
  ...
  try {
    ...
    // （生成CTAは1行が長くスマホで塊に見えるため。3行・各15字以内）
