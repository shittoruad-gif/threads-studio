...
const PENDING_TEXT_INPUT: Record<string, (q: Record<string, string>) => boolean> = {
  ...
  // 3案を見送った理由を文章で待っている状態。理由のボタンをもう一度押されたときは消さない
  skip_reason: (q) => q.a === "skipwhy",
...
export const HELP_TOPICS: Array<{ key: string; cat: string; q: string; a: string; action?: QuickItem; directPostback?: string }> = [
  ...
  { key: "safe", cat: "account", q: "アカウントを止められないために気をつけること",
    a: "新しいアカウントで最初から多く投稿すると、Meta側の自動判定で本人確認や停止が起きることがあります（2026年9月に実例）。Threads Studioは次を自動で守ります。\n" +
      "・連携から5日間は1日1件、10日間は1日2件まで（ご自身の投稿も数に入れます）。減った分は11日目以降に1日＋1件で補い、30日間の合計はご契約どおりになります\n" +
      "・その期間は投稿の末尾に誘導文を付けません（本文にURLは常に入れません）\n" +
      "・整体・整骨・鍼灸・エステなどでは、価格や「治る・歩けるようになった」などの結果表現を自動で言い換えます\n" +
      "・制限の兆候を見つけたら自動投稿を自動で止めてお知らせします\n\n" +
      "お願いしたいこと\n" +
      "1. 最初の2週間は、ご自身の投稿も1日1〜2件まで。フォローやいいねの連打はしない\n" +
      "2. 価格や結果の断定は書かない（入口は固定投稿とプロフィールのリンク）\n" +
      "3. プロフィールを先に整える（「お店・アカウント」→「プロフィールの提案」）\n" +
      "4. 本人確認が出たら正直に完了。停止なら異議申立は1回、結果まで投稿・フォロー・いいねを控え、復帰したらこのLINEで知らせる\n" +
      "5. Threadsアプリを日常的に開く（見るだけで大丈夫です）",
    action: { label: "プロフィールの提案", data: "c=proadv" } },
  { key: "next", cat: "account", q: "次に何をすればいい？（設定の進み具合）",
    a: "「次にやること」を押すと、設定の工程を順番に並べて、どこまで終わっていて次に何をするかをお見せします。\n" +
      "順番：お店の情報 → Threads連携 → ご案内先URL → プロフィール → 固定投稿（作る・公開・ピン留め）→ 自動投稿ON → 公開前の確認。\n" +
      "進んでいない工程がある間は、毎朝8時30分にも「次にやること」をお送りします（「この案内は不要」で止められます）。",
    action: { label: "次にやること", data: "m=next" } },
...
/**
 * 自動投稿が使えないプラン（フリー）の方への案内。
 *
 * ★2026-09-19 三上様「フリー／トライアルを有料に持っていきたい」。
 *   それまでは「ご利用中のフリープランでは自動投稿はご利用いただけません」＋URLの断り文で終わっていて、
 *   次の行動が無かった（本番の通し確認で、設定まで終えた津留様が投稿0本のまま止まっていた）。
 *   断りではなく「7日間無料で始めると、明日の朝6時に1本目が届く」という具体的な次を示す。
 *   数字の出どころ：無料期間は shared/plans.ts・料金ページの「7日間無料」、6時は autoPostScheduler の生成時刻。
 */
export function planUpgradeExitText(planName: string | null | undefined, base: string): string {
  return (
    `いまの${planName ?? "プラン"}では、毎日の自動投稿は動きません。\n\n` +
    "7日間無料でプランを始めると、明日の朝6時に、お店の情報をもとにした1本目がこのトークに届きます。\n" +
