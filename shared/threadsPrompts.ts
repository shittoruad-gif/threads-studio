...
7. **LINE誘導（URLはコメント欄）**：本文にURLは貼らない。公開時にシステムが公式LINEのURLを1件目のコメントとして自動で添付するので、本文の締めは「↓コメント欄のリンクからLINE登録できます」のようにコメント欄へ誘導する
   例：「LINE登録はコメント欄のリンクからどうぞ」
...
■ 文字数とフォーマット
- mainPost は400〜470文字でじっくり読ませる（普段の釣り投稿より長くてOK。固定投稿は読む気のある訪問者向けなので、フィード投稿の50字ルールは適用しない）
...
- ❌ 住所・営業時間・定休日・支払い方法の羅列（固定投稿は入口。詳細はLINEで案内する）
- ❌ 「※効果には個人差があります」等の免責・注意書きの列挙（規制対応は本文の表現自体を安全にすることで行い、免責の羅列で埋めない）
