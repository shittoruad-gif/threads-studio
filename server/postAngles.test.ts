...
import { POST_ANGLES, pickAngle, type AnglePerformance } from "../shared/postAngles";
...
function sample(stats: any, perf: AnglePerformance | undefined, n = 4000): Record<string, number> {
  ...
  for (let i = 0; i < n; i++) {
    const a = pickAngle(stats, rand, perf);
...
describe("切り口の重み付け", () => {
  ...
  it("実績データが空でも例外なく1つ返す", () => {
    const angle = pickAngle({}, seq([0.5]), { perAngle: {}, overallAvg: 0 });
