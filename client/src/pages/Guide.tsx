...
export default function Guide() {
  ...
  const quickStart = [
    {
      ...
      time: '30秒',
      body: 'サイドバー「Threads連携」→「アカウントを追加」を押して、Meta(Threads)の許可画面で承認するだけ。複数アカウントの切り替えもダッシュボード右上から1クリックでできます。',
    ...
    },
    {
      ...
      title: '投稿を生成 → 投稿',
      time: '1分',
      body: '投稿タイプ（地元ネタ型・実績型・共感型・ストーリー型など）を選んで「生成」。気に入ればそのまま投稿するか、予約投稿に登録するか、編集してから投稿します。',
  ...
  const faqs = [
    ...
    { q: 'ハッシュタグは入りますか？', a: 'Threadsではハッシュタグは効果が薄く、業者っぽさを出すので本ツールは原則入れません（仕様）。' },
    { q: '解約はどこからできますか？', a: 'サイドバー「設定」→「サブスクリプション」から手続き可能。日割り計算ではなく次回更新日まで利用できます。' },
