...
export async function detectNextAction(userId: number): Promise<NextAction | null> {
  ...
  if (usable.length === 0) {
    ...
    return {
      key: "no_project",
      text:
        // ★いちばん早い道（URLを貼る）を、ボタンより先に文章で出す（2026-09-21 三上様指示）。
        //   以前は「下の『はじめの設定』から」とだけ案内していたため、
        //   ボタンを2回押して3通目でやっと「URLを貼ってください」に着いた。
        //   URLを貼れば業種・地域・店名・強み・メニューが埋まるのに、
        //   その最短路がお客様に見えていなかった。
        "次にやることが1つあります。\n\n" +
        "まだ「お店の情報」が登録されていないため、投稿を作ることができません。\n\n" +
        "いちばん早いのは、お店のホームページのURLを、このトークにそのまま貼っていただくことです。\n" +
        "（ホットペッパー・Instagram・Googleマップのページでも大丈夫です）\n\n" +
        "ページを読んで、業種・地域・店名・強み・メニューを先に入れておきます。\n" +
        "残りは4問・2分ほどで終わります。\n\n" +
        "ホームページが無い場合は、下の「はじめの設定」からお進みください。",
