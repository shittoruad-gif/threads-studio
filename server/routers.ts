    ...
    generatePost: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const replyHookNote = input.withReplyHook
          ? '\n\n【返信誘発フレーズ（必須・Metaガイドライン厳守）】\n投稿の最後に、読者が一言で答えられる自然な問いかけを1文だけ追加すること。「あなたはどっち派？」「最後に行ったのはいつ？」のような体験・二択系が良い。★絶対禁止：「コメントで教えて」「いいねして」「フォローして」「シェアして」など、エンゲージメントを直接要求する言い方（Metaコミュニティガイドライン違反になる）。あくまで「自然に会話が起きる問いかけ」にすること。「〜だと思いませんか？」「いかがでしょうか？」などの曖昧な質問も禁止。'
          : '';
