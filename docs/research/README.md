...
- `2026-09-10/` … 歯科19本・工務店20本、ノウハウ／オーナーの悩み15本
  （検索語「整体院 集客」）。業種インサイトに `shika` / `koumuten` を追加。
- `2026-09-17/` … 脱毛サロン22本・居酒屋15本、ノウハウ10本（検索語「Threads 集客」）。
  両業種とも既存エントリ（`esthe` / `cafe`）が match でカバーしており、
  **業種インサイトの変更なし**（新候補は n=1・反応0で要検証に留めた）。
  ノウハウは3週連続で採用ゼロ。
- `2026-09-24/` … トリミングサロン17本（店13・飼い主3＋返信の要約）・行政書士13本。
...
「サロン 集客」（09-03）と「整体院 集客」（09-10）で2週続けて**採用ゼロ**だった。
この系統の検索語で伸びるのはノウハウではなく**店舗オーナー自身の弱音**で、
ノウハウ投稿は反応が一桁〜数十に留まり、内容も検証不能な実績と煽りが主体。
次週以降、「◯◯ 集客」系は1語までに減らし、残りは
**見込み客が実際に使う語**（業種名＋地域名、症状名、「おすすめ 教えて」など）に配分する。
そちらの方が win の素材（客が自分の言葉で語る選定軸）が取れている。

**2026-09-17の実施結果**: 上記の配分で実施（「Threads 集客」1語＋業種語2語）。
ノウハウ側は3週連続で採用ゼロ、一方で業種語の方は2業種とも
「反応が出ているのは店の告知ではなく**客側の投稿**（おすすめを教えて・体験談・
料金の内訳）」という同じ構図が再現した。この構図は3週で安定しているため、
次週以降は「◯◯ 集客」系のノウハウ収集を**隔週**に落とし、
空いた分を未収集業種（ペット・行政書士・学習塾以外の教室）に回す。
