...
export default function Guide() {
  ...
  const postModes = [
    ...
    },
    {
      ...
      body: '1日の投稿のうち1件として「@meta.ai 新倉敷・玉島で肩こりに悩む人に、整体に通うメリットを伝えて」のような投稿を朝〜昼に1件（1日の投稿のうち1件・プロ／ビジネスプランで利用可）。Meta AIが店名を出してコメントで答え、会話が付いて届く人が増える。依頼文は登録内容から決まった型で作る。※@meta.aiはThreads側で段階提供のため、まだ使えないアカウントもある。',
      when: '設定ONなら自動（既定ON）',
    },
    {
      title: '量産（変奏）',
  ...
  const troubleshooting = [
    ...
    },
    {
      ...
      a: 'Threadsでは誰でも投稿の返信欄で @meta.ai を呼べるため、お店の投稿にMeta AIの返信が付くことがあります。消したいときは返信を長押し→「非表示」（@meta.ai はブロック不可。ミュートと「興味がない」は使えます）。逆に「Meta AI呼びかけ投稿」に返事が付かない場合は、@meta.ai がそのアカウントでまだ提供されていない可能性があります（Threads側の段階提供）。使えるようになった時点でそのまま効き始めます。',
    },
    {
