...
【売れている演出ルール（第17回・最重要）】
お客さんは常に疑っている。「すごそうに見せる」演出を意図的に入れること：
- 「予約パンパン」「キャンセル待ち」「スレッズから〇人来店」などの売れてる演出を月10回以上発信
- 自動配信メッセージや投稿に「スレッズ見たって書いてくれたら〇〇」のオファーを織り込む
- ホットペッパー予約完了メールや申込数のスクショを定期発信（名前は伏せる）
- 「〇〇の申込が3日で50件超えました」など、数字で具体的に
- ヤラセ感を恐れない。安心感が買い物のスイッチを入れる
- 「私はもう200人入っている」「先月のキャンセル待ち6名」など、社会的証明を冒頭に置く

【心理トリガーの活用（第8回・第13回）】
投稿には以下のいずれか1つを必ず織り込む：
- 緊急性/限定性：「3日間だけ」「今日まで」「あと3名」「値上げ前」
- 損失回避：「行動しないと毎年100万損する」「やらないと気づかないうちに〜」
- 返金保証/リスク除去：「無料です」「予約しなくてもLINEだけで全部もらえる」
- 社会的証明：「すでに〇〇人が」「先月〇〇件」
- 権威性（プロらしさ）：「10年〇〇してきた立場で言うと」「〇〇を△△件見てきた」
- 行動する理由の明文化：なぜ今フォロー・LINE登録すべきかを毎回言葉にする
...
3. **誰のための場所か（一目でわかる自己紹介）**：店名・地域・職種・実績の順で短く
   例：「○○鍼灸院（横浜駅徒歩3分）です。10年で5000人を担当しました」
...
6. **LINE登録特典（理由付き）**：「迷う人向けに○○を無料配布しています」
   例：「【LINE登録特典】30秒でできる腰痛セルフチェック動画を配ってます」
