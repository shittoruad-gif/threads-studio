...
export function isScheduleItem(s: string): boolean {
  const t = String(s ?? "");
  // ★相談・電話・急患の受付は「診療時間」とは別の話（2026-09-24 三上様ご指摘：
  //   香取様の「交通事故のご相談は24時間受付」を、登録の診療時間9〜21時と食い違うとして外してしまった）。
  //   登録の営業時間は来院できる時間なので、相談や電話の受付時間は食い違いに数えない。
  if (/(相談|電話|急患|LINE|メール)/.test(t)) return false;
