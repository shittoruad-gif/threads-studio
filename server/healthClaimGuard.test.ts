...
/**
 * 2026-09-19 本番の通し確認。ログイン不要の体験ページ（tryGenerate）に検査が無く、
 * 「腰痛の根本原因は姿勢や骨盤の歪みです」という原因の言い切りがそのまま出ていた。
 * これまでの型は結果の断定だけで、診断（原因）の断定は見ていなかった。
 * 直近30日の健康系の公開投稿548本で新たに止まるのは1本だけ（和らげた形は通す）。
 */
describe("2026-09-19 追加：原因の断定", () => {
  // checkHealthClaims は引っかかった「文」を丸ごと落とす（2026-09-08 の方針：部分的な言い換えは日本語が壊れる）
  it("体験ページで実際に出た文を止め、その一文だけを落として前後は残す", () => {
    const v = checkHealthClaims("渋谷で腰痛に悩む方へ。\n\n腰痛の根本原因は姿勢や骨盤の歪みです。一時的なケアでは戻ってしまいます。\n国家資格者が根本から整えます。");
    expect(v.ok).toBe(false);
    expect(v.hits).toContain("原因の断定");
    expect(v.text).not.toContain("根本原因は姿勢や骨盤の歪みです");
    expect(v.text).toContain("渋谷で腰痛に悩む方へ。");
    expect(v.text).toContain("一時的なケアでは戻ってしまいます。");
  });

  it.each([
    "原因は体の使い方や姿勢です。",      // 本番 #1447（直近30日で唯一新たに止まる文）
    "原因は骨盤の歪みにあります。",
    "肩こりの正体は血流不足です",
  ])("言い切りを止める: %s", (t) => {
    expect(checkHealthClaims(t).hits).toContain("原因の断定");
  });
