...
## 2026-09-11 0時台（香取様「ここ数日は同じ内容」）

- 使い回し判定：14→10文字（漢字・カタカナ2字以上の一致のみ）、ご本人のThreads直近投稿も比較対象、最後の作り直しでも同じ言い回しは見送り（翌朝の自動補填へ）。
- 9:00 に個別お詫び6通（scheduled task `line-send-2026-09-11-shortfall-apology`、文面は scripts/ops/announcements/2026-09-11_shortfall_apology/）。
- デプロイ後に確認すること:
  - [ ] 9/11 6時の生成で `同じ言い回し` の見送りが出た枠数と、香取様（account 21）に2件届いたか
  - [ ] 9:00 の送信結果（6通の成功／失敗）
