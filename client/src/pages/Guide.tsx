...
export default function Guide() {
  ...
  const quickStart = [
    ...
    },
    {
      ...
      time: '約5分',
      body: '作成したプロジェクトで「カウンセリングを始める」を押すと、全13問の対話に入ります。ここで「使ってよい数字・実例・メニュー・よくある質問・原体験・特典」「絶対書きたくないこと」を一度答えると、AIがそれを丸ごと覚え、全ジャンルの投稿ネタを正確に作れるようになります（数字や架空のお客様の話を勝手に作りません）。',
  ...
  const accuracyFeatures = [
    {
      ...
      points: [
        '公式LINEなら最初は5つだけ（ホームページのURL1つと質問4つ・2分ほど）。残りは投稿が動き始めてから1日1問ずつ',
        'この画面（/ai-counseling）からまとめて入力する場合は全20問の対話形式（選択肢チップ・例文付き／「なし」ワンタップでスキップ可）',
        '業種を答えると、以降の選択肢・例文がその業種に合わせて切り替わります',
        '「実績数字なし」と答えれば AI も数字を書きません',
        '「絶対に書きたくないこと」リストは例外なく守られます',
        '文章の雰囲気は「目を引く書き方 / 落ち着いた書き方」から選べます（後から変更可）',
      ],
  ...
  const postModes = [
    ...
    },
    {
      ...
      body: '毎朝10時、公式LINEに「@meta.ai 新倉敷・玉島の名産品と言えば？」「@meta.ai 初めて整体院に行くとき、知っておくと安心なことは？」のような呼びかけ文が届く。「Threadsアプリで投稿する」を押すと文章入りの投稿画面が開くので「投稿」を押すだけ（自動投稿からだと@meta.aiがメンションにならないため、この1件だけアプリから）。Meta AIがコメントで答え、投稿の下に会話ができる（地元の話題・来店前の質問・体の質問を日替わり）。得意分野を登録すると、お店を紹介する日の文が「ダイエットに強い整体院」のように変わる。プロ／ビジネスプランで利用可。※@meta.aiはThreads側で段階提供のため、まだ使えないアカウントもある。',
      when: '毎朝10時（公式LINE連携が必要）',
    },
    {
      title: '量産（変奏）',
      tag: '当たり投稿の横展開',
  ...
  const troubleshooting = [
    ...
    {
      q: '投稿に @meta.ai の返信が付いた／Meta AIの返事が付かない',
      a: 'Threadsでは誰でも投稿の返信欄で @meta.ai を呼べるため、お店の投稿にMeta AIの返信が付くことがあります。消したいときは返信を長押し→「非表示」（@meta.ai はブロック不可。ミュートと「興味がない」は使えます）。逆に「Meta AI呼びかけ投稿」に返事が付かない場合は、①自動投稿（API）から出した投稿は@meta.aiがメンションにならず返事が付かない（毎朝10時のLINEのボタンからThreadsアプリで投稿してください）②@meta.ai がそのアカウントでまだ提供されていない（Threads側の段階提供）のどちらかです。',
    },
    {
  ...
  const glossary = [
    ...
    { term: 'AIカウンセリング', def: '全13問の対話で「使ってよい事実・メニュー・よくある質問・原体験」「絶対NG項目」をAIに教える前準備。捏造を防ぎ、全ジャンルのネタを正確に出す。' },
