...
### F. サーバーのディスク掃除（運営作業・2026-09-14 の朝の点検で発見）

- ディスクが **9/11 の35% → 9/14 に55%**（51G/99G）まで増えた。中身は毎晩のデプロイで積み上がる
  Docker のイメージ 41GB（再利用しない分 30GB）とビルドキャッシュ 16GB（同 6GB）。**データではない**。
- 夜間整備で一度掃除し、以後は定期実行にする（本番コンテナは止めずに済む）。
  ```
  ssh root@163.44.103.9 "docker image prune -af --filter until=168h; docker builder prune -af --filter until=168h; df -h /"
  ```
